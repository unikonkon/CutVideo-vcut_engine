...
export function StudioProvider({ children }: { children: ReactNode }) {
  ...
  const variant = useMemo<Variant>(
    () =>
      // ไทม์ไลน์/ชั้นแต่งใน .vcut เป็นของแบบที่ active เสมอ — ป้ายจึงต้องเป็นของแบบนั้น
      // ไม่ใช่แบบที่แค่กดดูอยู่ (?v=)
      eng.variants.find((v) => v.active) ??
      eng.variants.find((v) => v.id === r.variant) ??
      eng.variants[0] ?? {
        id: r.variant || "—",
        label: eng.proj?.project || "—",
        note: "",
        dur: 0,
        shots: 0,
        ready: false,
    ...
    [eng.variants, r.variant, eng.proj?.project],
  );
